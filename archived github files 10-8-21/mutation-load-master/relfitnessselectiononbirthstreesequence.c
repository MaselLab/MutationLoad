...
void InitializePopulation(tsk_table_collection_t *, tsk_id_t *, tsk_id_t *, long double *, long double *, int , double *, int, int, long double *);
...
void RecombineChromosomesIntoGamete(tsk_table_collection_t *, tsk_id_t *, tsk_id_t *, int, int, int, int , int , double *, double *, int, int *);
int SampleFromPoisson(float );
int DetermineNumberOfMutations(int, int, float);
void MutateGamete(tsk_table_collection_t *, tsk_id_t *, tsk_id_t, int , int , double *, double);
...
void ProduceMutatedRecombinedGamete(tsk_table_collection_t *, tsk_id_t *, tsk_id_t *, tsk_id_t *, int, int, double *, int , int , int , int , double , double , double , char * , double *, gsl_rng *);
void PerformOneTimeStep(tsk_table_collection_t *, tsk_id_t *, tsk_id_t *, int, int, int, long double *, long double *, double *, long double *, int, int, int, double, double, double, char *, double *, double *, gsl_rng *);
...
void InitializePopulation(tsk_table_collection_t * treesequencetablecollection, tsk_id_t * wholepopulationnodesarray, tsk_id_t * wholepopulationsitesarray, long double *wholepopulationwistree, long double *wholepopulationwisarray, int populationsize, double *populationgenomes, int totalpopulationgenomelength, int totaltimesteps, long double * psumofwis) {
    int i, j;
...
void RecombineChromosomesIntoGamete(tsk_table_collection_t * treesequencetablecollection, tsk_id_t * wholepopulationnodesarray, tsk_id_t * childnode, int totaltimesteps, int currenttimestep, int persontorecombine, int chromosomesize, int numberofchromosomes, double *gamete, double *populationgenomes, int totalindividualgenomelength, int * recombinationsites)
{
    ...
    *childnode = tsk_node_table_add_row(&treesequencetablecollection->nodes, 0, (totaltimesteps-currenttimestep), TSK_NULL, TSK_NULL, NULL, 0);
    check_tsk_error(*childnode);
    
    double childtime = treesequencetablecollection->nodes.time[*childnode];
    
    /*
    //Tree sequence recording doesn't like when the parent node occurs at the same time as the child node.
    //This is a bit of a hacky approach, but it should ensure that child nodes are always considered more recent than parent nodes.
    if (parenttime1 <= childtime) {
        treesequencetablecollection->nodes.time[*childnode] = (parenttime1 - 0.1);
        childtime = treesequencetablecollection->nodes.time[*childnode];
    }
    if (parenttime2 <= childtime) {
        treesequencetablecollection->nodes.time[*childnode] = (parenttime2 - 0.1);
        childtime = treesequencetablecollection->nodes.time[*childnode];
    }
    */
	...
	do {
        ...
        recombinationsites[h] = recombinationsite;
        
        if (startchromosome == 0) {
            returnvaluefortskit = tsk_edge_table_add_row(&treesequencetablecollection->edges, (h*chromosomesize), (h*chromosomesize + recombinationsite), wholepopulationnodesarray[parentnode1], *childnode);
            check_tsk_error(returnvaluefortskit);
            returnvaluefortskit = tsk_edge_table_add_row(&treesequencetablecollection->edges, (h*chromosomesize + recombinationsite), ((h+1)*chromosomesize), wholepopulationnodesarray[parentnode2], *childnode);
            check_tsk_error(returnvaluefortskit);
        } else {
            returnvaluefortskit = tsk_edge_table_add_row(&treesequencetablecollection->edges, (h*chromosomesize), (h*chromosomesize + recombinationsite), wholepopulationnodesarray[parentnode2], *childnode);
            check_tsk_error(returnvaluefortskit);
            returnvaluefortskit = tsk_edge_table_add_row(&treesequencetablecollection->edges, (h*chromosomesize + recombinationsite), ((h+1)*chromosomesize), wholepopulationnodesarray[parentnode1], *childnode);
            check_tsk_error(returnvaluefortskit);
        }
...
void MutateGamete(tsk_table_collection_t * treesequencetablecollection, tsk_id_t * wholepopulationsitesarray, tsk_id_t childnode, int chromosomesize, int numberofchromosomes, double *gamete, double mutationeffectsize)
{
    ...
    idofnewmutation = tsk_mutation_table_add_row(&treesequencetablecollection->mutations, wholepopulationsitesarray[mutatedsite], childnode, TSK_NULL, derivedstate, 10, NULL, 0);
...
void ProduceMutatedRecombinedGamete(tsk_table_collection_t *treesequencetablecollection, tsk_id_t * wholepopulationnodesarray, tsk_id_t * wholepopulationsitesarray, tsk_id_t * childnode, int totaltimesteps, int currenttimestep, double *wholepopulationgenomes, int chromosomesize, int numberofchromosomes, int totalindividualgenomelength, int currentparent, double deleteriousmutationrate, double beneficialmutationrate, double Sb, char * beneficialdistribution, double *parentgamete, gsl_rng * randomnumbergeneratorforgamma)
{
    ...
    RecombineChromosomesIntoGamete(treesequencetablecollection, wholepopulationnodesarray, childnode, totaltimesteps, currenttimestep, currentparent, chromosomesize, numberofchromosomes, parentgamete, wholepopulationgenomes, totalindividualgenomelength, recombinationsites);
    ...
    for (k = 0; k < numberofdeleteriousmutations; k++) {
        MutateGamete(treesequencetablecollection, wholepopulationsitesarray, *childnode, chromosomesize, numberofchromosomes, parentgamete, -Sds[k]);
    }
    ...
    if (strcmp(beneficialdistribution, "point") == 0) {
        for (k = 0; k < numberofbeneficialmutations; k++) {
            MutateGamete(treesequencetablecollection, wholepopulationsitesarray, *childnode, chromosomesize, numberofchromosomes, parentgamete, Sb);
        }
    } else if (strcmp(beneficialdistribution, "exponential") == 0) {
        for (k = 0; k < numberofbeneficialmutations; k++) {
            generatedSb = gsl_ran_exponential(randomnumbergeneratorforgamma, Sb);
            MutateGamete(treesequencetablecollection, wholepopulationsitesarray, *childnode, chromosomesize, numberofchromosomes, parentgamete, generatedSb);
        }
...
void PerformOneTimeStep(tsk_table_collection_t *treesequencetablecollection, tsk_id_t * wholepopulationnodesarray, tsk_id_t * wholepopulationsitesarray, int popsize, int totaltimesteps, int currenttimestep, long double *wholepopulationwistree, long double *wholepopulationwisarray, double *wholepopulationgenomes, long double * psumofwis, int chromosomesize, int numberofchromosomes, int totalindividualgenomelength, double deleteriousmutationrate, double beneficialmutationrate, double Sb, char * beneficialdistribution, double *parent1gamete, double *parent2gamete, gsl_rng * randomnumbergeneratorforgamma)
{
    ...
    ProduceMutatedRecombinedGamete(treesequencetablecollection, wholepopulationnodesarray, wholepopulationsitesarray, &childnode1, totaltimesteps, currenttimestep, wholepopulationgenomes, chromosomesize, numberofchromosomes, totalindividualgenomelength, currentparent1, deleteriousmutationrate, beneficialmutationrate, Sb, beneficialdistribution, parent1gamete, randomnumbergeneratorforgamma);
    ProduceMutatedRecombinedGamete(treesequencetablecollection, wholepopulationnodesarray, wholepopulationsitesarray, &childnode2, totaltimesteps, currenttimestep, wholepopulationgenomes, chromosomesize, numberofchromosomes, totalindividualgenomelength, currentparent2, deleteriousmutationrate, beneficialmutationrate, Sb, beneficialdistribution, parent2gamete, randomnumbergeneratorforgamma);
    ...
    wholepopulationnodesarray[currentvictim*2] = childnode1;
    wholepopulationnodesarray[currentvictim*2 + 1] = childnode2;
...
{
    int i, j, k, returnvaluefortskfunctions;
    ...
    InitializePopulation(&treesequencetablecollection, wholepopulationnodesarray, wholepopulationsitesarray, wholepopulationwistree, wholepopulationwisarray, popsize, wholepopulationgenomes, totalpopulationgenomelength, totaltimesteps, psumofwis);
...
double BisectionMethodToFindSbWithZeroSlope(double * Sb1, double * Sb2, char * Nxtimestepsname, char * popsizename, char * delmutratename, char * chromsizename, char * chromnumname, char * mubname, char * typeofrun, int Nxtimesteps, int popsize, int chromosomesize, int numberofchromosomes, double deleteriousmutationrate, double beneficialmutationrate, double slopeforcontourline, char * beneficialdistribution, gsl_rng * randomnumbergeneratorforgamma, FILE * veryverbosefilepointer, FILE * verbosefilepointer, FILE * miscfilepointer) {
    ...
    if (((slope1 - slopeforcontourline)*(slopemid - slopeforcontourline)) >= 0.0) {
        fprintf(miscfilepointer, "Zero slope root not bracketed properly, with starting slopes %.10f and %.10f\n", slope1, slopemid);
        return 0.0;
    }
    ...
    for (i = 1; i <= maxtries; i++) {
        ...
        if (fabs(factor) < accuracy || Sbmid == 0.0) {
            return root;
        }
